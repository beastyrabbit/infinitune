...
import { ArrowLeft, Search, X, SlidersHorizontal, Music, Play, Pause, Volume2, VolumeX } from 'lucide-react'
import { api } from '../../convex/_generated/api'
import { CoverArt } from '@/components/autoplayer/CoverArt'
import { TrackDetail } from '@/components/autoplayer/TrackDetail'
import { getGlobalAudio, playerStore, setCurrentSong, setPlaying, setDuration, setVolume, toggleMute } from '@/lib/player-store'
...
function matchesSearch(song: any, term: string): boolean {
  if (!term) return true
  const lower = term.toLowerCase()
  const fields = [song.title, song.artistName, song.genre, song.subGenre, song.description, song.lyrics]
  return fields.some((f) => f && String(f).toLowerCase().includes(lower))
}

function matchesFilters(song: any, filters: Filters, sessionMap: Map<string, string>): boolean {
  if (filters.genres.length > 0 && !filters.genres.includes(song.genre)) return false
  if (filters.moods.length > 0 && !filters.moods.includes(song.mood)) return false
  if (filters.energies.length > 0 && !filters.energies.includes(song.energy)) return false
  if (filters.eras.length > 0 && !filters.eras.includes(song.era)) return false
  if (filters.languages.length > 0 && !filters.languages.includes(song.language)) return false
  if (filters.ratings.length > 0) {
    const rating: RatingFilter = song.userRating === 'up' ? 'liked' : song.userRating === 'down' ? 'disliked' : 'unrated'
    if (!filters.ratings.includes(rating)) return false
  }
  if (filters.sessions.length > 0) {
    const sessionName = sessionMap.get(song.sessionId) || song.sessionId
...
function MiniPlayer({ currentSong }: { currentSong: { _id: string; title?: string; artistName?: string } | null }) {
  const { isPlaying, currentTime, duration, volume, isMuted } = useStore(playerStore)
  ...
  return (
    <div className="flex items-center gap-3 border-t-2 border-white/10 px-4 py-2 bg-black/60">
      ...
      <div className="shrink-0 min-w-0 max-w-[140px]">
        <p className="text-[10px] font-black uppercase truncate">{currentSong.title || '...'}</p>
        <p className="text-[9px] uppercase text-white/30 truncate">{currentSong.artistName || '...'}</p>
      </div>

      {/* Progress bar */}
      <div className="flex-1 flex items-center gap-2 min-w-0">
        <span className="text-[10px] font-bold text-white/40 shrink-0">{formatTime(currentTime)}</span>
        <div
        ...
        <span className="text-[10px] font-bold text-white/40 shrink-0">{formatTime(duration)}</span>
      </div>
...
function LibraryPage() {
  ...
  const filterOptions = useMemo(() => {
    if (!songs) return { genres: [], moods: [], energies: [], eras: [], languages: [], sessions: [] }
    return {
      ...
      sessions: unique(songs.map((s) => sessionMap.get(s.sessionId) || s.sessionId)),
    }
  }, [songs, sessionMap])
  ...
  const filtered = useMemo(() => {
    if (!songs) return []
    return songs.filter((s) => matchesSearch(s, debouncedSearch) && matchesFilters(s, filters, sessionMap))
  }, [songs, debouncedSearch, filters, sessionMap])

  // Counts per filter value (based on search + other filters, not this filter)
  const filterCounts = useMemo(() => {
    if (!songs) return { genres: new Map(), moods: new Map(), energies: new Map(), eras: new Map(), languages: new Map(), ratings: new Map(), sessions: new Map() }

    function countFor(key: keyof Filters, valueExtractor: (s: any) => string | undefined | null) {
      const otherFilters = { ...filters, [key]: [] }
      const base = songs!.filter((s) => matchesSearch(s, debouncedSearch) && matchesFilters(s, otherFilters, sessionMap))
    ...
    return {
      genres: countFor('genres', (s) => s.genre),
      moods: countFor('moods', (s) => s.mood),
      energies: countFor('energies', (s) => s.energy),
      eras: countFor('eras', (s) => s.era),
      languages: countFor('languages', (s) => s.language),
      ratings: countFor('ratings', (s) => s.userRating === 'up' ? 'liked' : s.userRating === 'down' ? 'disliked' : 'unrated'),
      sessions: countFor('sessions', (s) => sessionMap.get(s.sessionId) || s.sessionId),
    }
  }, [songs, debouncedSearch, filters, sessionMap])
  ...
  const hasActiveFilters = search || Object.values(filters).some((arr) => arr.length > 0)
  ...
  const activeFilterCount = Object.values(filters).reduce((sum, arr) => sum + arr.length, 0) + (search ? 1 : 0)
        ...
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {activeFilterCount > 0 && (
                <span className="bg-red-500 text-white text-[10px] font-black px-1.5 py-0.5">{activeFilterCount}</span>
              )}
            </button>

...
import { formatElapsed, LiveTimer, StatusBadge, CollapsibleJson } from '@/components/autoplayer/test/shared'
...
function AceTestPage() {
  ...
  const [caption, setCaption] = useState('upbeat electronic dance, synthesizer leads, driving bass, energetic drums')
      ...
      pollRef.current = setInterval(async () => {
        ...
        try {
          ...
          if (pollData.status === 'succeeded') {
            ...
            const aceStepUrl = settings?.aceStepUrl || 'http://192.168.10.120:8001'
  ...
  }, [lyrics, caption, bpm, keyScale, timeSignature, duration, aceModel, settings, stopPolling])
  ...
  const statusMap: Record<TaskStatus, 'pending' | 'running' | 'done' | 'error'> = {
    idle: 'pending',
    submitting: 'running',
    running: 'running',
    succeeded: 'done',
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      ...
      <section className="border-4 border-white/10 bg-black">
        ...
        <div className="p-4 space-y-4">
          <div>
            <label className="text-xs font-bold uppercase text-white/40 mb-1 block">Caption</label>
            <input
          ...
          <div>
            <label className="text-xs font-bold uppercase text-white/40 mb-1 block">Lyrics</label>
            <textarea
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <label className="text-xs font-bold uppercase text-white/40 mb-1 block">BPM</label>
              <input
                type="number"
                className="w-full h-8 rounded-none border-4 border-white/20 bg-gray-900 font-mono text-xs text-white px-2 focus:outline-none focus:border-yellow-500"
                value={bpm}
            ...
            <div>
              <label className="text-xs font-bold uppercase text-white/40 mb-1 block">Key</label>
              <input
                className="w-full h-8 rounded-none border-4 border-white/20 bg-gray-900 font-mono text-xs text-white px-2 focus:outline-none focus:border-yellow-500"
                value={keyScale}
            ...
            <div>
              <label className="text-xs font-bold uppercase text-white/40 mb-1 block">Time Sig</label>
              <input
                className="w-full h-8 rounded-none border-4 border-white/20 bg-gray-900 font-mono text-xs text-white px-2 focus:outline-none focus:border-yellow-500"
                value={timeSignature}
            ...
            <div>
              <label className="text-xs font-bold uppercase text-white/40 mb-1 block">Duration (s)</label>
              <input
                type="number"
                className="w-full h-8 rounded-none border-4 border-white/20 bg-gray-900 font-mono text-xs text-white px-2 focus:outline-none focus:border-yellow-500"
                value={duration}
      ...
      {/* STATUS PANEL */}
      {taskStatus !== 'idle' && (
        <section className={`border-4 bg-black ${
          taskStatus === 'failed' ? 'border-red-500/40'
            : taskStatus === 'succeeded' ? 'border-green-600/30'
          ...
          <div className="p-4 space-y-2">
            {taskId && (
              <div className="text-[10px] font-bold uppercase text-white/30">
                TASK ID: <span className="text-white/60 font-mono">{taskId}</span>
              </div>
            )}
            ...
            {audioUrl && (
              <div className="mt-4">
                <span className="text-[10px] font-black uppercase text-white/40 block mb-2">AUDIO</span>

			...
			try {
				...
				const acePromise = (async () => {
					if (startIdx <= 2) {
						...
						if (!aceRes.ok)
							throw new Error(`ACE-Step submit failed: ${await aceRes.text()}`);
			...
			<section className="border-4 border-white/10 bg-black">
				...
				<div className="p-4 space-y-4">
					...
					<div className="text-[10px] font-bold uppercase text-white/30 space-y-1">
						<p>
							PROVIDER: {settings?.textProvider || "ollama"} | MODEL:{" "}
							...
							{songMeta && (
								<div className="flex-1 space-y-1">
									<p className="text-lg font-black uppercase">
										{String(songMeta.title)}
									</p>
									<p className="text-sm font-bold uppercase text-white/50">
										{String(songMeta.artistName)}
									</p>
									<p className="text-xs font-bold uppercase text-white/30">
										{String(songMeta.genre)} / {String(songMeta.subGenre)}
									</p>
									<p className="text-xs font-bold uppercase text-white/30">
										{String(songMeta.bpm)} BPM | {String(songMeta.keyScale)} |{" "}
										{String(songMeta.timeSignature)}
									</p>
								</div>
							)}
						</div>

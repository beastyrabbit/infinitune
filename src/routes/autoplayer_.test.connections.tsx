...
function ConnectionsTestPage() {
  ...
  const testService = useCallback(async (service: ServiceName) => {
    setStates((prev) => ({
      ...prev,
      [service]: { status: 'testing', message: null, responseTime: null, models: null },
    }))
    ...
    try {
      ...
      if (data.ok) {
        ...
        setStates((prev) => ({
          ...prev,
          [service]: { status: 'ok', message: data.message, responseTime, models },
        }))
      } else {
        setStates((prev) => ({
          ...prev,
          [service]: { status: 'error', message: data.error, responseTime, models: null },
        }))
      }
    } catch (e: any) {
      setStates((prev) => ({
        ...prev,
        [service]: { status: 'error', message: e.message, responseTime: Date.now() - startedAt, models: null },
      }))
    }
  }, [settings])
              ...
              <div className="p-4 space-y-3">
                ...
                {state.responseTime !== null && (
                  <div className="text-[10px] font-bold uppercase text-white/30">
                    Response: <span className="text-white/60">{state.responseTime}ms</span>
                  </div>
                )}

                {state.message && (
                  <p className={`text-[10px] font-bold uppercase ${state.status === 'error' ? 'text-red-400' : 'text-green-400'}`}>
                    {state.message}
                  </p>
                )}
                ...
                {state.models && state.models.length > 0 && (
                  <div>
                    <span className="text-[10px] font-black uppercase text-white/40 block mb-1">MODELS ({state.models.length})</span>
                    <div className="flex flex-wrap gap-1">
                      {state.models.map((m) => (
                        <span key={m} className="text-[10px] font-mono px-1.5 py-0.5 bg-white/5 border border-white/10 text-white/50">
                          {m}
                        </span>
                      ))}
                    </div>
...
function StatusPill({ status }: { status: ServiceStatus }) {
  ...
  return (
    <span className={`px-2 py-0.5 text-[10px] font-black uppercase ${styles[status]}`}>
      {status}
    </span>
  )
}

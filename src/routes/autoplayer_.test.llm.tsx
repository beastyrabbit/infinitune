...
function LlmTestPage() {
  ...
  useEffect(() => {
    if (settings) {
      const p = settings.textProvider === 'openrouter' ? 'openrouter' : 'ollama'
      setProvider(p)
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      ...
      <section className="border-4 border-white/10 bg-black">
        ...
        <div className="p-4 space-y-4">
          <div>
            <label className="text-xs font-bold uppercase text-white/40 mb-1 block">Prompt</label>
            <textarea
              ...
              <div className="flex gap-2">
                <button
                  className={`flex-1 h-8 border-4 font-mono text-[10px] font-black uppercase ${
                    provider === 'ollama' ? 'border-yellow-500 bg-yellow-500/10 text-yellow-500' : 'border-white/10 text-white/40'
                  }`}
      ...
      <section className="border-4 border-white/10 bg-black">
        ...
        <div className="p-4 space-y-1">
          ...
          <div className="mt-2 text-[10px] font-bold uppercase text-white/20">
            Structured output: {provider === 'ollama' ? 'Ollama format (JSON schema)' : 'OpenRouter response_format (json_schema)'}
          </div>
        </div>
      </section>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-black uppercase truncate max-w-xs">
                    {gen.result?.title || gen.prompt.slice(0, 40)}
                  </span>
                </div>
                  ...
                  {gen.result && (
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                        <div>
                          <span className="font-bold uppercase text-white/40">Title: </span>
                          <span className="font-black">{gen.result.title}</span>
                        </div>
                        <div>
                          <span className="font-bold uppercase text-white/40">Artist: </span>
                          <span className="font-black">{gen.result.artistName}</span>
                        </div>
                        <div>
                          <span className="font-bold uppercase text-white/40">Genre: </span>
                          <span>{gen.result.genre}</span>
                        </div>
                        <div>
                          <span className="font-bold uppercase text-white/40">Sub-genre: </span>
                          <span>{gen.result.subGenre}</span>
                        </div>
                        <div>
                          <span className="font-bold uppercase text-white/40">BPM: </span>
                          <span>{gen.result.bpm}</span>
                        </div>
                        <div>
                          <span className="font-bold uppercase text-white/40">Key: </span>
                          <span>{gen.result.keyScale}</span>
                        </div>
                        <div>
                          <span className="font-bold uppercase text-white/40">Time Sig: </span>
                          <span>{gen.result.timeSignature}</span>
                        </div>
                        <div>
                          <span className="font-bold uppercase text-white/40">Duration: </span>
                          <span>{gen.result.audioDuration}s</span>
                        </div>
                      </div>

                      {/* Lyrics */}
                      <div>
                        <span className="text-[10px] font-black uppercase text-white/40">LYRICS</span>
                      ...
                      <div className="space-y-2">
                        <div>
                          <span className="text-[10px] font-black uppercase text-white/40">CAPTION: </span>
                          <span className="text-xs text-white/60">{gen.result.caption}</span>
                        </div>
                        <div>
                          <span className="text-[10px] font-black uppercase text-white/40">COVER PROMPT: </span>
                          <span className="text-xs text-white/60">{gen.result.coverPrompt}</span>
                        </div>
                      </div>

...
type TestStatus = { state: 'idle' } | { state: 'testing' } | { state: 'ok'; message: string } | { state: 'error'; message: string }
...
export function SettingsDialog({ open, onOpenChange }: SettingsDialogProps) {
  ...
  const [openrouterTest, setOpenrouterTest] = useState<TestStatus>({ state: 'idle' })
  ...
  const testConnection = useCallback(async (provider: string) => {
    const setStatus = provider === 'ollama' ? setOllamaTest
      : provider === 'openrouter' ? setOpenrouterTest
      : provider === 'comfyui' ? setComfyuiTest
      : setAceTest
  ...
  const TestButton = ({ provider, status }: { provider: string; status: TestStatus }) => (
    <div className="flex items-center gap-2">
      ...
      {status.state === 'ok' && (
        <span className="text-[10px] font-bold uppercase text-green-400">{status.message}</span>
      )}
      {status.state === 'error' && (
        <span className="text-[10px] font-bold uppercase text-red-400">{status.message}</span>
      )}
    </div>
  )
        ...
        <div className="space-y-6 mt-4">
            ...
            <div className="space-y-3">
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs font-bold uppercase text-white/40">Provider</label>
                  {textProvider === 'ollama' && <TestButton provider="ollama" status={ollamaTest} />}
                  {textProvider === 'openrouter' && <TestButton provider="openrouter" status={openrouterTest} />}
                </div>
              ...
              <div>
                ...
                {textProvider === 'ollama' && textModels.length > 0 ? (
                  ...
                  <Input
                    ...
                    placeholder={textProvider === 'openrouter' ? 'GOOGLE/GEMINI-2.5-FLASH' : 'LLAMA3.1:8B'}
                    value={textModel}
          ...
          <section>
            <div className="flex items-center justify-between mb-3 border-b-2 border-white/10 pb-1">
              ...
              {imageProvider === 'comfyui' && <TestButton provider="comfyui" status={comfyuiTest} />}
              {imageProvider === 'openrouter' && <TestButton provider="openrouter" status={openrouterTest} />}
            </div>
            <div className="space-y-3">
              <div>

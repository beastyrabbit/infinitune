...
export async function repairIncompleteRadioAlbums(): Promise<InventoryRepairResult> {
	...
	for (const album of albumRows) {
		...
		if (albumTrackRepairCount > 0) repairedAlbums++;
...
export async function topUpInventory(opts?: { force?: boolean }) {
	const force = opts?.force === true;
	const repair = await repairIncompleteRadioAlbums();
	const repairedStats = getInventoryStats();
	if (!force && repairedStats.activeListenerCount <= 0) {
		return {
			created: 0,
			...repair,
			stats: repairedStats,
	...
	const stats = repairedStats;
	const target = stats.inventoryTarget;
	const desiredActiveAlbums = force ? target + 1 : target;
	const createCount = force
		? Math.min(
				1,
				Math.max(0, desiredActiveAlbums - stats.untouchedActiveAlbums),
			)
		: Math.max(0, desiredActiveAlbums - stats.untouchedActiveAlbums);

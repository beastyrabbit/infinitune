...
/**
 * Upper bound for per-album track counts. Albums are 12 tracks, so any
 * single mix component above this is meaningless and a very large value
 * (corrupt setting or hostile API input) would make buildTrackTypeMix
 * allocate a huge array before truncating — clamp it here.
 */
const MAX_TRACKS_PER_ALBUM = 12;

function parseCount(value: string | undefined, fallback: number): number {
	const parsed = Number.parseInt(value ?? "", 10);
	if (!Number.isInteger(parsed) || parsed < 0) return fallback;
	return Math.min(parsed, MAX_TRACKS_PER_ALBUM);
}

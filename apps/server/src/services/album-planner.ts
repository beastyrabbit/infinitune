...
export type RadioTrackType = "cover" | "new" | "cover-of-cover";

const TRACK_TYPES: RadioTrackType[] = ["cover", "new", "cover-of-cover"];
...
export function buildTrackTypeMix(
	...
	for (let i = 0; i < settings.randomFill; i++) {
		types.push(TRACK_TYPES[Math.floor(rand() * TRACK_TYPES.length)]);
	}
...
const TrackPlanSchema = z.object({
	...
	type: z.enum(["cover", "new", "cover-of-cover"]),
	title: z.string().min(1),
